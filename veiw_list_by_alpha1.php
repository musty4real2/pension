...
echo "<h1>OUTSTANDING PAYMENT RECORD SORTED ALPHABETHICALLY</h1>";

$sql="SELECT * FROM `voters` WHERE `payment_status`='outstanding'  ORDER BY `name_of_beneficiary` ASC ";
...
if ($current_page != 1) {
 echo '<center><a href="veiw_list_by_alpha1.php?s=' .
...
for ($i = 1; $i <= $pages; $i++) {
 if ($i != $current_page) {
 echo '<a href="veiw_list_by_alpha1.php?s=' .
(($display * ($i - 1))) . '&p=' .
$pages . '">' . $i . '</a> ';
 } else {
 echo $i . ' ';
