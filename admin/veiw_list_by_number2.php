...
<title>LOCAL GOVERNMENT PENSION DEPARTMENT ::::::::view list by number</title>
...
echo "<h1> RECORD of OUTSTANDING PAYMENTS DONE SORTED BY NUMBER(S)</h1>";

$sql="SELECT * FROM `voters` WHERE `payment_status`='outstanding' ORDER BY `id` ASC ";
$query=mysql_query($sql) or die("ERROR: ".mysql_error());

if(mysql_num_rows($query)==0){
	echo "<h2>No Records to be Displayed</h2>";
	}
...
<td><?php echo "<a href='signout.php?statuus=$status&vid={$row['id']}'> Update Payment</a>";?></td>
</tr>
...
<?php
number_format();
$query3="SELECT SUM(amount) FROM `voters` WHERE `payment_status`='outstanding'";
...
if ($current_page != 1) {
 echo '<center><a href="veiw_list_by_number2.php?s=' .
...
for ($i = 1; $i <= $pages; $i++) {
 if ($i != $current_page) {
 echo '<a href="veiw_list_by_number2.php?s=' .
(($display * ($i - 1))) . '&p=' .
$pages . '">' . $i . '</a> ';
 } else {
 echo $i . ' ';
