...
echo "<h1> RECORD of PAYMENTS DONE SORTED BY NUMBER(S)</h1>";

$sql="SELECT * FROM `voters` WHERE `payment_status`='paid' ORDER BY `id` ASC ";
...
if ($current_page != 1) {
 echo '<center><a href="veiw_list_by_number1.php?s=' .
...
for ($i = 1; $i <= $pages; $i++) {
 if ($i != $current_page) {
 echo '<a href="veiw_list_by_number1.php?s=' .
(($display * ($i - 1))) . '&p=' .
$pages . '">' . $i . '</a> ';
 } else {
 echo $i . ' ';
