...
echo "<h1> RECORD SORTING NUMBER(S)</h1>";

$sql="SELECT * FROM `records` ORDER BY `id` ASC ";
...
if ($current_page != 1) {
 echo '<center><a href="veiw_list_by_number.php?s=' .
...
for ($i = 1; $i <= $pages; $i++) {
 if ($i != $current_page) {
 echo '<a href="veiw_list_by_number.php?s=' .
(($display * ($i - 1))) . '&p=' .
$pages . '">' . $i . '</a> ';
 } else {
 echo $i . ' ';
